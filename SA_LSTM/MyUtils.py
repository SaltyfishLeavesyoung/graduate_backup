...
def adjustment(ROIs, labels):
    ...
    temoff[temoff < -0.055] = temoff[temoff < -0.055] * 0 - 0.055
    ROIs = labels + temoff
    ROIs[ROIs < 0] = ROIs[ROIs < 0] * 0 + 0
    ROIs[ROIs > 1] = ROIs[ROIs > 1] * 0 + 1
    return ROIs
...
def get_coarse_errors(coordinates1, lables):
    ...
    coordinates1_b[:, :, 0] = coordinates1_b[:, :, 0] * (256 - 1)
    coordinates1_b[:, :, 1] = coordinates1_b[:, :, 1] * (256 - 1)
    coordinates1_b[:, :, 2] = coordinates1_b[:, :, 2] * (8 - 1)

    lables_b[:, :, 0] = lables_b[:, :, 0] * (256 - 1)
    lables_b[:, :, 1] = lables_b[:, :, 1] * (256 - 1)
    lables_b[:, :, 2] = lables_b[:, :, 2] * (8 - 1)
...
def getcropedInputs(ROIs, inputs_origin, cropSize, useGPU):
    ...
    b, c, l, h, w = int(b), int(c), int(l), int(h), int(w) 
    # l, h, w = 576, 768, 768
    cropSize = cropSize / 2
    ...
    for landmarkId in range(landmarkNum):
        ...
        lz, uz, lx, ux, ly, uy = int(z - cropSize), int(z + cropSize), int(x - cropSize), int(x + cropSize), int(y - cropSize), int(y + cropSize)
        lzz, uzz, lxx, uxx, lyy, uyy = max(lz, 0), min(uz, l), max(lx, 0), min(ux, h), max(ly, 0), min(uy, w)

        # ~ print (z, x, y)
        # ~ print ("boxes ", lz, uz, lx, ux, ly, uy)
        cropedDICOM = inputs_origin[:, :, int(lzz): int(uzz), int(lxx): int(uxx), int(lyy): int(uyy)]
        # ~ print ("check before", cropedDICOM.size())
        if lz < 0:
            _, _, curentZ, curentX, curentY = cropedDICOM.size()
            curentZ, curentX, curentY = int(curentZ), int(curentX), int(curentY)
            temTensor = torch.zeros(b, c, int(0 - lz), curentX, curentY)
            if useGPU >= 0: temTensor = temTensor.cuda(useGPU)
            cropedDICOM = torch.cat((temTensor, cropedDICOM), 2)
        if uz > l:
            _, _, curentZ, curentX, curentY = cropedDICOM.size()
            curentZ, curentX, curentY = int(curentZ), int(curentX), int(curentY)
            temTensor = torch.zeros(b, c, int(uz - l), curentX, curentY)
            if useGPU >= 0: temTensor = temTensor.cuda(useGPU)
            cropedDICOM = torch.cat((cropedDICOM, temTensor), 2)
        if lx < 0:
            _, _, curentZ, curentX, curentY = cropedDICOM.size()
            curentZ, curentX, curentY = int(curentZ), int(curentX), int(curentY)
            temTensor = torch.zeros(b, c, curentZ, int(0 - lx), curentY)
            if useGPU >= 0: temTensor = temTensor.cuda(useGPU)
            cropedDICOM = torch.cat((temTensor, cropedDICOM), 3)
        if ux > h:
            _, _, curentZ, curentX, curentY = cropedDICOM.size()
            curentZ, curentX, curentY = int(curentZ), int(curentX), int(curentY)
            temTensor = torch.zeros(b, c, curentZ, int(ux - h), curentY)
            if useGPU >= 0: temTensor = temTensor.cuda(useGPU)
            cropedDICOM = torch.cat((cropedDICOM, temTensor), 3)
        if ly < 0:
            _, _, curentZ, curentX, curentY = cropedDICOM.size()
            curentZ, curentX, curentY = int(curentZ), int(curentX), int(curentY)
            temTensor = torch.zeros(b, c, curentZ, curentX, int(0 - ly))
            if useGPU >= 0: temTensor = temTensor.cuda(useGPU)
            cropedDICOM = torch.cat((temTensor, cropedDICOM), 4)
        if uy > w:
            _, _, curentZ, curentX, curentY = cropedDICOM.size()
            curentZ, curentX, curentY = int(curentZ), int(curentX), int(curentY)
            temTensor = torch.zeros(b, c, curentZ, curentX, int(uy - w))
            if useGPU >= 0: temTensor = temTensor.cuda(useGPU)
