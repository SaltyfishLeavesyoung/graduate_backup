...
class Tester(object):
    ...
    def test(self, net=None):
        ...
        for img, landmark, mask, guassian_mask, offset_y, offset_x, landmark_list in tqdm(self.dataloader_1):
            ...
            image_gt = visualize(img, landmark_list, [i for i in range(37)]) #blue
            image_gt_pred = visualize(torch.from_numpy(np.array(image_gt).transpose(2,0,1)).unsqueeze(0), pred)
            image_gt_pred.save(os.path.join(self.output_dir, str(ID)+'hand_gt_pred.png'))
            image_gt.save(os.path.join(self.output_dir, str(ID)+'hand_gt.png'))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--iteration", default='1', help="position of the output dir")
    ...
    #checkpoints = torch.load(os.path.join(config['runs_dir'], \
    #                    "model_best.pth"))
    checkpoints = torch.load('/root/NEW_UNCERTAINTY/runs/2023-04-15-23:01:01.575591/model_best.pth')
