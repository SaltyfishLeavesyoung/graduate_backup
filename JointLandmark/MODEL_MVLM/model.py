...
class MVLMModel(BaseModel):
    def __init__(self, n_landmarks=6, n_features=256, dropout_rate=0.2):
        super().__init__()
        ...
        self.fc = nn.Linear(n_landmarks*128*128, n_landmarks * 3)
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        outputs = torch.sigmoid(self.fc(x))
        outputs = outputs.view(outputs.size(0), -1, 3)
