...
class Evaluater(object):
    ...
    def record(self, pred, landmark):
        ...
        for i in range(b):
            ...
            self.dis1.append(np.abs(np.sqrt(np.power(pred[i][3][0] * ratio[0] - pred[i][5][0] * ratio[0], 2) + np.power(pred[i][3][1] * ratio[1] - pred[i][5][1] * ratio[1], 2)) - np.sqrt(np.power(landmark[i][3][0] - landmark[i][5][0], 2) + np.power(landmark[i][3][1] - landmark[i][5][1], 2))))
    ...
    def cal_metrics(self):
        ...
        self.logger.info("dis1 {}".format(np.array(self.dis1).mean()))
        self.logger.info("dis2 {}".format(np.array(self.dis1).std()))
