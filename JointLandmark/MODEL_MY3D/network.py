...
class UNet_Pretrained_Att(nn.Module):
    ...
    def forward(self, x):
        ...
        '''node = f.reshape(f.shape[0], f.shape[1], -1)
        attn, v = self.sa(node)
        out = torch.bmm(attn, v).unsqueeze(2)
        out = out.unsqueeze(2)
        out = self.conv1(out)'''
        ...
        '''out = self.conv1(features[3])'''

        '''node = self.mtn(f)
        gcn_out, adj = self.gcn(node)
        out = self.new_fc(gcn_out)
        N, C, _ = out.shape
        out = out.reshape(N, C, 1, 16, 16)
        out = self.conv1(out)'''
        ...
        x1 = x.view(x.size(0), self.n_classes, -1)
        x1 = self.fc(x1)
        x1 = torch.sigmoid(x1)

        return x, x1, sa_attn, adj
