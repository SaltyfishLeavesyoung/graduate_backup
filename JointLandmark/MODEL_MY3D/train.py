...
from fa68pt3D import fa68pt3D, fa68pt3D_2, fa68pt3D_3
...
def main(args):
    ...
    train_loader = torch.utils.data.DataLoader(
        fa68pt3D('/home/yeziyang/Desktop/JointLandmark/train.txt', train=True, depth_res=args.depth_res,
            nStack=args.stacks, sigma=args.sigma),
        batch_size=args.train_batch, shuffle=True,
        num_workers=args.workers, pin_memory=True)

    """train_loader_2 = torch.utils.data.DataLoader(
        fa68pt3D_2('/home/yeziyang/Desktop/JointLandmark/val_300.txt', train=True, depth_res=args.depth_res,
            nStack=args.stacks, sigma=args.sigma),
        batch_size=args.train_batch, shuffle=True,
        num_workers=args.workers, pin_memory=True)

    train_loader_3 = torch.utils.data.DataLoader(
        fa68pt3D_3('/home/yeziyang/Desktop/JointLandmark/train.txt', '/home/yeziyang/Desktop/JointLandmark/val_300.txt', train=True, depth_res=args.depth_res,
            nStack=args.stacks, sigma=args.sigma),
        batch_size=args.train_batch, shuffle=True,
        num_workers=args.workers, pin_memory=True)"""
    ...
    model = UNet_Pretrained_Att(8, args.num_landmarks, adj)
    model = torch.nn.DataParallel(model).cuda()

    if args.pretrain:
        checkpoint = torch.load('/home/yeziyang/Desktop/JointLandmark/MODEL_MY3D/checkpoint/base_191.tar')
...
def run(best_acc, model, data_loader, mode, criterion, optimizer):
    ...
    for i, (inputs, target, meta) in enumerate(data_loader):
        ...
        # run forward
        _, pred_coord, attn, adj = model.forward(input_var)
        #print(pred_coord[0])
        if i == 0:
            print(attn[0])
            print(adj)
        ...
        if mode == 'train':
            loss_coord = criterion(pred_coord[:,:,0], coord_var[:,:,0] / 255.0) * 255.0 / 7.0 + criterion(pred_coord[:,:,1], coord_var[:,:,1] / 255.0) * 255.0 / 7.0 + criterion(pred_coord[:,:,2], coord_var[:,:,2] / 7.0)
            # loss_coord *= 10
            optimizer.zero_grad()
            loss_coord.backward()
            optimizer.step()
            ...
            loss_coord = criterion(pred_coord[:,:,0], coord_var[:,:,0] / 255.0) * 255.0 / 7.0 + criterion(pred_coord[:,:,1], coord_var[:,:,1] / 255.0) * 255.0 / 7.0 + criterion(pred_coord[:,:,2], coord_var[:,:,2] / 7.0)
            # loss_coord *= 10
        pred_landmarks = (torch.mul(pred_coord[:, 0:6, :], RATIO).data).cpu()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpus', default='3,4,5', type=str, help='set gpu IDs')
    # Training strategy
    parser.add_argument('-j', '--workers', default=4, type=int, metavar='N',
                        help='number of data loading workers (default: 4)')
    parser.add_argument('--epochs', default=500, type=int, metavar='N',
                        help='number of total epochs to run')
    parser.add_argument('--start-epoch', default=500, type=int, metavar='N',
