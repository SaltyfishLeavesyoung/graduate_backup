...
def to_Image(tensor, show=None, normalize=False):
    ...
    image = to_PIL(tensor)
    if show:
        image.save("./paper_figure/"+show + ".png")
    return image
...
def voting_channel(k, heatmap, regression_y, regression_x,\
    ...
    # argsort返回数组值从小到大的索引值，[-10:]即argsort后的后10个，即数组值最大的10个的索引值
...
def voting(heatmap, regression_y, regression_x, Radius):
    ...
    # 在heatmap的19个通道上，取每个通道的前num_candi个值最大(即概率最大)的点，num_candi即regression的像素面积
...
def visualize(img, landmarks, red_marks=None, blue_marks=None, ratio=0.01, highlight=55):
    ...
    for i, landmark in enumerate(landmarks):
        ...
        draw.text((landmark[0]-Radius, landmark[1]-Radius), str(id%37 + 1), \
            fill=red, font=font)
