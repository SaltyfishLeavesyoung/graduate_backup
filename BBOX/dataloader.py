...
ISBI_PATH = "/home/yeziyang/Desktop/IEEE ISBI 2015 Challenge"
ISBI_LANDMARK = 19
SIZE = [800, 640]
R_RATIO = 0.05
...
class Cephalometric(data.Dataset):
    ...
    def __init__(self, pathDataset, mode, num_landmark=19, size=[800, 640]):
    ...
    def __getitem__(self, index):
        ...
        offset_x_local = torch.zeros((self.num_landmark, y, x), dtype=torch.float)
        offset_y_local = torch.zeros((self.num_landmark, y, x), dtype=torch.float)
        ...
        for i, landmark in enumerate(landmark_list):
            ...
            for j in range(x):
                offset_x_local[i][:, j] = landmark[0] - j
            for k in range(y):
                offset_y_local[i][k, :] = landmark[1] - k
        offset_x_local = offset_x_local / x
        offset_y_local = offset_y_local / y
        
        return item['image'], landmarks, mask, guassian_mask, offset_y_local, offset_x_local, offset_y, offset_x, landmark_list
