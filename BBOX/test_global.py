...
class Tester(object):
    def __init__(self, logger, config, g_net=None, tag=None, train="", args=None):
    ...
    def test(self, g_net=None, l_net=None):
        ...
        for img, landmark, mask, guassian_mask, offset_y_local, offset_x_local, _, _, landmark_list in tqdm(self.dataloader_1):
            ...
            pred_landmark = coord_from_heatmap.cpu()
...
if __name__ == "__main__":
    ...
    tester = Tester(logger, config, g_net, args.tag, args.train, args)
    if args.debug != '':
        tester.debug()
    else:
        tester.test()
